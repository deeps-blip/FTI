...
from intelligence.feature_vector import extract_features
...
def score_risk(
    ...
    # ------------------------------------------------
    fl_model = FederatedRiskScorer()
    if fl_model.is_available():
        feature_vector = extract_features(intents, obfuscation, behavior_flow, dynamic_findings)
        fl_score = fl_model.predict_risk(feature_vector)
        if fl_score is not None:
            verdict = (
                "high_risk_malware" if fl_score >= 70 else
                "suspicious_artifact" if fl_score >= 40 else
                "low_confidence_threat"
            )
            severity = classify_severity(fl_score)
            
            # Estimate static/dynamic components for backward compatibility
            static_component = fl_score if not dynamic_findings else fl_score * 0.6
            dynamic_component = 0 if not dynamic_findings else fl_score * 0.4
            
            return {
                "score": fl_score,
                "severity": severity,
                "verdict": verdict,
                "rationale": ["Score generated by Federated Risk Scorer model"],
                "risk_model": "federated_risk_scorer_v1",
